...
- `internal/smpp/egress/` - outbound carrier SMPP pool: per-carrier session bind/rebind, decrypts SMPP password at connect.
...
- Schema is applied manually via `make schema DB_URL=...`; it is never auto-applied at startup.
