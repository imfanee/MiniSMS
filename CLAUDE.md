...
- `internal/smpp/egress/` - outbound carrier SMPP pool: per-carrier `sessionGroup` of N parallel ESME binds (`carriers.smpp_bind_count`, 1..16, editable in the admin carrier SMPP panel), round-robin `submit_sm`, decrypts SMPP password at connect. A `deliver_sm` DLR may land on any bind; correlation is carrier-wide by `carrier_message_id` (`dlr.HandleCarrierSMPP` -> `db.ResolveSMSLogMessageID`). Changing the bind count rebinds within ~60s (manager refresh tick). A carrier can run several parallel binds to its upstream SMSC. Session events stream to a per-carrier in-memory ring buffer (`internal/smpp/egresslog`, no secrets, no disk); the admin carrier SMPP panel has an `Open SMPP logs` button that live-tails them over SSE (`GET /admin/carriers/{id}/smpp-logs` + `/stream`, gated by `PermCarriersView`, GET-only so CSRF-exempt, `X-Accel-Buffering: no` for nginx, strict per-response CSP nonce on the popup). `Manager.BindStatus` feeds the live "X/Y binds up" shown in the panel Session status; `Manager.Restart` (POST `/admin/carriers/{id}/smpp-restart`, `PermCarriersEdit` + CSRF, button on the panel and in the log popup) stops and immediately rebinds a carrier's session group for troubleshooting. The web layer reaches these via the `web.SMPPController` interface (so `web` does not import `egress`; `*egress.Manager` is assigned in `main`).
- **SMPP ingress (`internal/smpp/server/`)** has the mirror-image admin features: `server.Server` owns its own `egresslog.Hub` (keyed by client_id) and emits bind accepted/rejected, `submit_sm` accepted/rejected, `deliver_sm` DLR sent, and unbind/disconnect events; sessions are removed from the registry on disconnect so `BindCount` is accurate. The client SMPP panel shows the live binds-connected count, an `Open SMPP logs` popup (`GET /admin/clients/{id}/smpp-logs` + `/stream`, `PermClientsView`), and a `Restart SMPP` button (`POST /admin/clients/{id}/smpp-restart`, `PermClientsEdit` + CSRF, audited `client.smpp_restart`) that force-disconnects the client's binds. `web.SMPPIngressController` (`BindCount`, `RestartClient`) decouples `web` from `server`; `*server.Server` is assigned in `main`. The carrier and client log popups share one template (`smpp_logs`, fields `Title`/`StreamURL`/`RestartURL`) and the `streamSMPPLogs` SSE core.
- **Current SMPP password is shown in both SMPP panels** (decrypted into the password input, revealed by the Show toggle) per operator request; admin-only, permission-gated, HTTPS.
- **SMPP command_status decoding + deep-logging toggle:** `internal/smpp/smppstatus.Describe(code)` maps a command_status to the ESME_* mnemonic + a plain-English cause (shared by egress and ingress); bind/submit failures log the decoded code so an SMSC rejection (e.g. `0x0D ESME_RBINDFAIL`) is never confused with a network/firewall block. The go-smpp library returns the bind response status as a `pdu.Status` error, so `egress.bind()` returns `(rejected, code, err)`. The log popup has a `deep logs` checkbox (`POST /admin/{carriers|clients}/{id}/smpp-logs/debug`, `Perm*View` + CSRF) that flips a per-entity flag in `egresslog.Hub` (`SetDebug`/`Debug`, 30-min auto-expiry); sessions emit `DEBUG` per-PDU detail only while it is on.
...
- **DLR ingest (multi-bit dlr-mask):** `db.UpdateDLRReceived` applies only while no final status (delivered/undelivered/rejected) is stored and returns whether it applied; an intermediate receipt (gateway SMSC ACK / queued from `dlr-mask=31`) must never block or overwrite the final. Keep `dlr.IsFinalStatus` / `shouldForwardDLR` semantics (forward on first receipt, then again only on reaching a final state).
...
- **Sensitive / business data must never appear in committed code or docs (this is a standing rule, not a one-off).** This repository is published to GitHub. Do NOT put any of the following in tracked files (code, comments, docs, tests, example envs, scripts): real carrier, client, gateway, vendor, or destination names/brands; real MSISDNs or sender IDs; real IP addresses or hostnames; credentials, system IDs, or DSNs; and never describe a current security weakness of the deployment (open ports, missing firewall, disabled controls, etc.). Keep everything generic and placeholder-based (for example `the carrier`, `a Kannel-style gateway`, `+10000000000`, `CHANGEME`, `your-host`). Operator-specific topology, identifiers, and any security caveats belong only in the local agent memory (`/root/.claude/.../memory/`, not in git) or operator-side notes. When asked to record such facts, put them in memory, not in the repo.
