...
func GetSMSLogForDLR(ctx context.Context, pool *pgxpool.Pool, messageID string) (*DLRMessage, error) {
	...
	err := pool.QueryRow(ctx, `
		SELECT sl.message_id::text, sl.client_ref, sl.to_number, sl.from_number, sl.segments, sl.total_charged::text,
			ca.name, sl.failover_sequence, sl.received_at, sl.status,
			sl.dlr_requested, sl.dlr_webhook_url,
			sl.source_addr_ton, sl.source_addr_npi, sl.dest_addr_ton, sl.dest_addr_npi,
			c.dlr_webhook_secret, ca.dlr_inbound_secret, ca.dlr_message_id_field, ca.dlr_status_field, ca.dlr_status_map
		FROM sms_logs sl
		...
		&m.MessageID, &m.ClientRef, &m.ToNumber, &m.FromNumber, &m.Segments, &m.TotalCharged,
		&m.CarrierName, &m.FailoverSequence, &m.ReceivedAt, &m.Status,
		&m.DLRRequested, &m.DLRWebhookURL,
		&m.SourceAddrTON, &m.SourceAddrNPI, &m.DestAddrTON, &m.DestAddrNPI,
		&m.ClientWebhookSecret, &m.CarrierInboundSecret, &m.CarrierDLRMessageIDField, &m.CarrierDLRStatusField, &statusMapRaw,
	)
	if err != nil {
		return nil, err
	}
