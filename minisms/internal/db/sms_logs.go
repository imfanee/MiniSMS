...
func CreateSMSLog(ctx context.Context, tx pgx.Tx, in SMSLog) (string, error) {
	...
	err := tx.QueryRow(ctx, `
		...
		in.SourceAddrTON, in.SourceAddrNPI, in.DestAddrTON, in.DestAddrNPI,
	).Scan(&id)
	return id, err
}

func MarkSMSAccepted(ctx context.Context, tx pgx.Tx, messageID, carrierID string, failoverSequence int, carrierMessageID, carrierResponseBody string, carrierResponseCode int, tonNPI *[4]int16) error {
